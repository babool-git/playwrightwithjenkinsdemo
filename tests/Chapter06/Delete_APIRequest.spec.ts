...
const patchAPIResponse = await request.patch(`/booking/${bookingid}`, {
  ...
  expect(await patchAPIResponse.status()).toBe(200);
    expect((await patchAPIResponse).statusText()).toBe('OK');

    const putAPIJSONresponse = (await patchAPIResponse).json();
    console.log('PATCH API Response : ' + JSON.stringify(patchAPIResponse, null, 2));

    console.log(
  'PATCH API Response : ' +
  JSON.stringify(patchAPIResponse, null, 2)
);
...
const deleteAPIResponse = await request.delete(`/booking/${bookingid}`, {
  ...
  expect(await deleteAPIResponse.status()).toBe(201);
    expect((await deleteAPIResponse).statusText()).toBe('Created');
